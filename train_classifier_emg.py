...
def main():
    ...
    if args.action == "train":
        ...
        val_loader = torch.utils.data.DataLoader(ActionVisionDataset(args.dataset.shift.split("-")[-1],
                                                                     modalities,
                                                                     'val', 
                                                                     args.dataset, 
                                                                     args.train.num_frames_per_clip,
...
def validate(model, val_loader, device, it, num_classes):
    ...
    with torch.no_grad():
        ...
        # if y != 0 else 0
        class_accuracies = [(x / y) * 100  if y != 0 else 0 for x, y in zip(model.accuracy.correct, model.accuracy.total)]
